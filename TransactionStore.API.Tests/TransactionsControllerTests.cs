...
public class TransactionsControllerTests
{
    ...
    [Fact]
    public async Task GetTransactionsById_IdSent_OkResultReceieved()
    {
        //arrange
        var id = new Guid();
        _transactionsServiceMock.Setup(x => x.GetTransactionsByIdAsync(id)).ReturnsAsync(new List<TransactionWithAccountIdResponse>());
        var controller = new TransactionsController(_transactionsServiceMock.Object);

        //act
        var actual = await controller.GetTransactionsById(id);

        //assert
        actual.Result.Should().BeOfType<OkObjectResult>();
        _transactionsServiceMock.Verify(m => m.GetTransactionsByIdAsync(id), Times.Once);
    }
}
